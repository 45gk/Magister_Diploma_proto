...
## DWH: переход на Data Vault

В проекте сохранены **старые DDL-таблицы** (для совместимости), и дополнительно добавлены **новые Data Vault 2.0 таблицы** в `scripts/init.sql` с префиксом `dv_`:
- Hubs: `dv_hub_client`, `dv_hub_application`;
- Link: `dv_link_client_application`;
- Satellites: `dv_sat_client_profile`, `dv_sat_financial_profile`, `dv_sat_behavioral_context`, `dv_sat_application_details`, `dv_sat_scoring_result`, `dv_sat_agent_explanation`, `dv_sat_default_status`.

Это позволяет постепенно переводить ETL и витрины на Data Vault, не ломая текущую интеграцию web/ml/agent сервисов.
